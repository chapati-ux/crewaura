...
// Normalizes gallery.js entries into { image, text } items for the WebGL
// canvas. gallery.js items look like { img, title, media, src, ... } —
// `img` is always the static thumbnail/poster (for both images and videos),
// so it's what we display here regardless of `media` type. Also tolerates
// plain string imports or {src, alt}-style objects in case the source shape
// changes later.
function normalizeGalleryItems(source) {
  if (!source || !source.length) return [];
  return source.map((item, i) => {
    if (typeof item === 'string') {
      return { image: item, text: `Photo ${i + 1}` };
    }
    return {
      image: item.img || item.src || item.image || item.default || item.url || item,
      text: item.title || item.alt || item.text || item.tag || `Photo ${i + 1}`
    };
  });
}
...
class Media {
  ...
  onResize({ screen, viewport } = {}) {
    ...
    // Scale cards down further on narrow viewports so more of the gallery
    // is visible at once and text/cards don't dominate the screen on mobile.
    const mobileFactor = this.screen.width < 640 ? 0.7 : this.screen.width < 1024 ? 0.85 : 1;
...
class App {
  ...
  createMedias(items, bend = 1, textColor, borderRadius, font) {
    // Prefer explicitly passed `items`, then the locally imported gallery
    // images, then a small placeholder set as a last-resort fallback.
    const localGalleryItems = normalizeGalleryItems(images);
    ...
    const galleryItems =
      items && items.length ? items : localGalleryItems.length ? localGalleryItems : placeholderItems;
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    resolveFont(font, fontUrl).then(resolvedFont => {
      ...
      // Re-run layout whenever the container itself changes size, not just
      // the window — covers sidebar toggles, orientation changes on some
      // devices, and any responsive layout shift the parent triggers.
